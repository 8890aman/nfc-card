  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-gray-100 to-white">
        <motion.div
          className="absolute top-0 left-0 w-64 h-64 bg-green-200 rounded-full filter blur-3xl opacity-50"
          animate={{
            x: [0, 100, 0],
            y: [0, 50, 0],
            scale: [1, 1.1, 1],
        ...
        />
        <motion.div
          className="absolute bottom-0 right-0 w-96 h-96 bg-green-300 rounded-full filter blur-3xl opacity-50"
          animate={{
            x: [0, -150, 0],
            y: [0, -100, 0],
            scale: [1, 1.2, 1],
        ...
        />
        <motion.div
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-green-100 rounded-full filter blur-3xl opacity-50"
          animate={{
            scale: [1, 1.3, 1],
          }}
          transition={{
      ...
      >
        <div className="w-full max-w-4xl">
          ...
          >
            <motion.div
              className="absolute inset-0 bg-gradient-to-br from-green-100 to-green-300 opacity-70"
          ...
          {/* Name */}
          <motion.h2
            className="text-2xl font-bold text-center mb-2 text-green-700"
          ...
          >
            <p className="text-sm text-center mb-2 text-green-600">{userData.tagline}</p>
            <p className="text-sm text-center text-green-700">{userData.bio}</p>
          </motion.div>
          ...
          >
            {tags.map((tag, index) => (
              <motion.span
                key={index}
                className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full m-1"
                variants={fadeInUpVariants}
          ...
          >
            {[
              ...
              <motion.a
                ...
                className="w-8 h-8 border border-green-600 rounded-full flex items-center justify-center hover:bg-green-600 hover:text-white transition-colors duration-300"
                variants={fadeInUpVariants}
...
const NavButton = ({ icon: Icon, rotate = false, isSelected = false, onClick }) => (
  ...
  >
    <motion.div
      className={`p-3 rounded-full ${isSelected ? 'bg-green-600' : 'bg-green-100'} group-hover:bg-green-200 transition-all duration-300 ${rotate ? 'transform rotate-45' : ''}`}
      whileHover={{ rotate: rotate ? 90 : 0 }}
    >
      <Icon className={`text-2xl ${isSelected ? 'text-white' : 'text-green-600'} group-hover:text-green-700 transition-colors duration-300`} />
    </motion.div>
  </motion.button>
);
